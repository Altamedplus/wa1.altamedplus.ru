...
export const Mask = {
    phoneMasks: [
        ...
        },
        {
            ...
            "name_en": "Emergency",
            "desc_en": "",
            "name_ru": "Телефон экстренной помощи",
            "desc_ru": ""
        }
    ],
    init: function () {
        //Mask
        let i:any, def:any, val:any, pattern:any, mask__data:any = this.phoneMasks;
        function mask(e:any) {
            if (e.target.tagName !== "INPUT") return false;
            if (!e.target.dataset.phonemask) return false;
            if (!mask__data.length) return false;

            for (let obj of mask__data) {
                let regex2 = new RegExp('^' + obj.mask.replace(/\D+/g, ""));
                if (e.target.value.replace(/\D+/g, "").match(regex2)) {
                    pattern = obj.mask;
                }
            }
            ...
            if (e.target.value.length == pattern.length) return false;
            if (e.type == "focusout" && val.length <= def.length) e.target.value = null;
            e.target.value = pattern.replace(/./g, function(a:any) {
                return /[#\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? "" : a;
            });
        }

        document.addEventListener("input", mask);
        document.addEventListener("focusin", mask);
        document.addEventListener("focusout", mask);

        var event = document.createEvent('Event');
        event.initEvent('input', true, true);
        let maskInputs = document.querySelectorAll('[data-phonemask]');
        for(let input of maskInputs)
